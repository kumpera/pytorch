...
hardening:
get should not bomb if a key is missing


misc:
cache alloc_buffer or, at least, make it return smaller sizes
use shared ptr for UvClient and LibUVStoreDaemon
collect stats on command partial parsing failures
  if significant, do more upfront checking to ensure enough data is available
make backlog configurable
Make StreamWriter::send block futher data adding
...
class StreamWriter {
 ...
 public:
  ...
  static StreamWriter* from_write_request(uv_write_t* req) {
    ...
    return reinterpret_cast<StreamWriter*>(
        reinterpret_cast<char*>(req) - offset);
  }
};
