...
class DeviceMeshCollectiveTest(DTensorTestBase):
    ...
    def test_all_reduce_1d(self):
        ...
        # We have to clone the resulting tensor because assertEqual fails to compare AsyncTensor with plan tensor.
    ...
    def test_all_reduce_nd(self):
        ...
        for dim, dim_group in enumerate(dim_to_subgroups):
            ...
            print(f"--doing {dim} / {dim_group} over ranks: {get_process_group_ranks(dim_group)}---")
            torch.distributed.barrier()
            dim_group_size = get_world_size(dim_group)
            # fixme should just call get_process_group_ranks
            global_ranks = [
                get_global_rank(dim_group, i) for i in range(dim_group_size)
            ]
            cloned_local_tensor = mesh.all_reduce(local_tensor, mesh_dim=dim).clone()
            res_num = sum(global_ranks)
            print(f"{torch.distributed.get_rank()} local:{local_tensor} got:{cloned_local_tensor} expected:{res_num} sum --> {torch.equal(cloned_local_tensor.cpu(), torch.ones(3, 3) * res_num)}")
